...
9. 只把**完成且测试通过**的工作 merge 回 main（main 保持绿色）。练习未完成时留在分支上即可。
10. 合并后用 `bash scripts/remove-worktree.sh <分支名> [--delete-branch]` 清理。有未提交改动时 git 会拒绝删除——先提交或明确丢弃，不要习惯性 `--force`。
